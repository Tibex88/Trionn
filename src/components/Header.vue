...
<template>
  ...
  <div class="flex justify-between items-center p-5 h-fit w-screen fixed z-20 top-0">

    <svg xmlns="http://www.w3.org/2000/svg" 
        width="100" 
        height="19" 
        class="fill-black dark:fill-porcelain"
        fill-rule="evenodd" 
    ...
    <div class="flex gap-2 ">
      ...
      <button 
        ...
        class="cursor-pointer transition-all duration-500 aspect-square rounded-full w-10  bg-slate-600/30">
        <fa :class="{'hidden' : modelValue}" icon="moon" />
        <fa :class="{'hidden' : !modelValue}" icon="sun" />
      </button>
      
      <button 
        @click="soundOn = !soundOn" 
        class="cursor-pointer transition-all duration-500 aspect-square rounded-full w-10  bg-slate-600/30">
        <fa :class="{'hidden' : !soundOn}" icon="volume-high" />
        <fa :class="{'hidden' : soundOn}" icon="volume-xmark" />
      </button>
    </div>
    ...
    <div class="flex items-center gap-2 font-bold ">
      ...
      <div 
        @click="menu = !menu"
        class="aspect-square rounded-full w-10 bg-slate-600/30 cursor-pointer">
        
        <div class="w-full h-full p-2 flex items-end justify-center flex-col gap-1">
          <span class="w-5 border border-black dark:border-white"></span>
          <span class="w-2 border border-black dark:border-white"></span>
        </div>
      </div>
  ...
  <nav 
  :class="{'hidden' : !menu}"
  class="nav fixed z-10 bg-porcelain dark:bg-woodsmoke flex mb-20 flex-col gap-20 justify-center items-start p-5 w-screen h-screen">
    
    <div class="group/list flex flex-col text-5xl font-bold"> 
      
      <!-- <router-link to='/'> 
          <span class="dark:bg-porcelain bg-black w-0 group-hover:w-5 aspect-square rounded-full hidden group-hover:block transition-all duration-700  "></span>
          <span> Home </span>
      </router-link> -->

      <div v-for="(route,index) in routes">
        <router-link :to="route.link" @click="menu = false" class="group/link: hover:translate-x-8 transition-all duration-1000 flex gap-2 items-center">
          <span class="dark:bg-porcelain bg-black w-0  group-hover/link:w-5 aspect-square rounded-full hidden group-hover:block transition-all duration-700 "></span>
          <span class="hover:text-french_pass"> {{route.name}} </span>
        </router-link>
    ...
    <div class="flex flex-col text-lg gap-4">
      ...
      <div class="flex gap-0">
        <!-- icons -->
        <img class="w-10" src="../assets/dribble.svg" alt="">
        <img class="w-10"  src="../assets/linkedin.svg" alt="">
        <img class="w-10"  src="../assets/instagram.svg" alt="">
        <img class="w-10"  src="../assets/behance.svg" alt="">
        <img class="w-10"  src="../assets/facebook.svg" alt="">
      </div>
    </div>
